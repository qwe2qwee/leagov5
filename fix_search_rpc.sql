...
-- Create improved search_cars function with all fixes and correct type casting
...
BEGIN
  ...
  INTO v_total_count
  FROM cars c
  JOIN cars_with_details cwd ON c.id = cwd.id
  ...
  SELECT
    ...
    c.fuel_type,
    c.seats,
    cwd.actual_available_quantity as available_quantity,
    cwd.features_ar,
    cwd.features_en,
    c.rental_types,
    (CASE
      ...
      ELSE (
        CASE WHEN cwd.model_name_en ILIKE '%' || p_search_query || '%' THEN 2.0 ELSE 0.0 END +
        CASE WHEN cwd.model_name_ar ILIKE '%' || p_search_query || '%' THEN 2.0 ELSE 0.0 END +
        CASE WHEN cwd.brand_name_en ILIKE '%' || p_search_query || '%' THEN 1.5 ELSE 0.0 END +
        CASE WHEN cwd.brand_name_ar ILIKE '%' || p_search_query || '%' THEN 1.5 ELSE 0.0 END
      )
